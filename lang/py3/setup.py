...
def SetupSources():
  ...
  """
    ...
    shutil.copy(
        src=version_file_path, dst=os.path.join(py3_dir, "avro", VERSION_FILE_NAME)
    )
    ...
    shutil.copy(
        src=avsc_file_path, dst=os.path.join(py3_dir, "avro", "HandshakeRequest.avsc")
    )
    ...
    shutil.copy(
        src=avsc_file_path, dst=os.path.join(py3_dir, "avro", "HandshakeResponse.avsc")
    )

    avsc_file_path = os.path.join(root_dir, "share", "test", "schemas", "interop.avsc")
    shutil.copy(
        src=avsc_file_path, dst=os.path.join(py3_dir, "avro", "tests", "interop.avsc")
    )
