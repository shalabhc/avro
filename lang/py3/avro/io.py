...
def Validate(expected_schema, datum):
  ...
  """
    ...
    if schema_type == "null":
        ...
        return isinstance(datum, int) and (INT_MIN_VALUE <= datum <= INT_MAX_VALUE)
    elif schema_type == "long":
        return isinstance(datum, int) and (LONG_MIN_VALUE <= datum <= LONG_MAX_VALUE)
        ...
        return isinstance(datum, list) and all(
            Validate(expected_schema.items, item) for item in datum
        )
    elif schema_type == "map":
        return (
            isinstance(datum, dict)
            and all(isinstance(key, str) for key in datum.keys())
            and all(Validate(expected_schema.values, value) for value in datum.values())
        )
    elif schema_type in ["union", "error_union"]:
        return any(
            Validate(union_branch, datum) for union_branch in expected_schema.schemas
        )
...
class DatumReader(object):
    ...
    def match_schemas(writer_schema, reader_schema):
        ...
        if "union" in [w_type, r_type] or "error_union" in [w_type, r_type]:
            ...
            w_type in schema.PRIMITIVE_TYPES
            and r_type in schema.PRIMITIVE_TYPES
            and w_type == r_type
        ):
            return True
        elif w_type == r_type == "record" and DatumReader.check_props(
            writer_schema, reader_schema, ["fullname"]
        ):
            return True
        elif w_type == r_type == "error" and DatumReader.check_props(
            writer_schema, reader_schema, ["fullname"]
        ):
            return True
        elif w_type == r_type == "request":
            return True
        elif w_type == r_type == "fixed" and DatumReader.check_props(
            writer_schema, reader_schema, ["fullname", "size"]
        ):
            return True
        elif w_type == r_type == "enum" and DatumReader.check_props(
            writer_schema, reader_schema, ["fullname"]
        ):
            return True
        elif w_type == r_type == "map" and DatumReader.check_props(
            writer_schema.values, reader_schema.values, ["type"]
        ):
            return True
        elif w_type == r_type == "array" and DatumReader.check_props(
            writer_schema.items, reader_schema.items, ["type"]
        ):
            return True
    ...
    """
        ...
        while block_count != 0:
            ...
            for i in range(block_count):
                read_items.append(
                    self.read_data(writer_schema.items, reader_schema.items, decoder)
                )
            block_count = decoder.read_long()
        return read_items
        ...
        while block_count != 0:
            ...
            for i in range(block_count):
                ...
                read_items[key] = self.read_data(
                    writer_schema.values, reader_schema.values, decoder
                )
            block_count = decoder.read_long()
        return read_items
                ...
                if field_name not in writers_fields_dict:
                    if field.has_default:
                        ...
                        raise SchemaResolutionException(
                            fail_msg, writer_schema, reader_schema
                        )
        return read_record
