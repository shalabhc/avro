...
class Protocol(object):
    ...
    """
        ...
        request_schema = Message._ParseRequestFromJSONDesc(
            request_desc=request_desc, names=names
        )
    ...
    def _ParseMessageDescMap(message_desc_map, names):
        for name, message_desc in message_desc_map.items():
            yield Protocol._ParseMessageDesc(
                name=name, message_desc=message_desc, names=names
            )

    def __init__(self, name, namespace=None, types=tuple(), messages=tuple()):
    ...
    """
        ...
        assert len(self._types) == len(
            self._type_map
        ), "Type list %r does not match type map: %r" % (self._types, self._type_map)
        ...
        self._message_map = MappingProxyType(
            {message.name: message for message in self._messages}
        )
...
class Message(object):
    ...
    """
        error_union_desc = {"type": schema.ERROR_UNION, "declared_errors": errors_desc}
...
def ProtocolFromJSONData(json_data):
  ...
  """
    ...
    types = tuple(
        map(lambda desc: Protocol._ParseTypeDesc(desc, names=names), type_desc_list)
    )

    message_desc_map = json_data.get("messages", dict())
    messages = tuple(Protocol._ParseMessageDescMap(message_desc_map, names=names))

    return Protocol(name=name, namespace=namespace, types=types, messages=messages)
