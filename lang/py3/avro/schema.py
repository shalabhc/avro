...
PRIMITIVE_TYPES = frozenset([NULL, BOOLEAN, STRING, BYTES, INT, LONG, FLOAT, DOUBLE])

NAMED_TYPES = frozenset([FIXED, ENUM, RECORD, ERROR])

VALID_TYPES = frozenset.union(
    PRIMITIVE_TYPES, NAMED_TYPES, [ARRAY, MAP, UNION, REQUEST, ERROR_UNION]
)
...
FIELD_RESERVED_PROPS = frozenset(["default", "name", "doc", "order", "type"])

VALID_FIELD_SORT_ORDERS = frozenset(["ascending", "descending", "ignore"])
...
class Names(object):
    ...
    """
        if schema.fullname in VALID_TYPES:
            raise SchemaParseException("%s is a reserved type name." % schema.fullname)
        if schema.fullname in self.names:
            raise SchemaParseException("Avro name %r already exists." % schema.fullname)
...
class EnumSchema(NamedSchema):
    def __init__(
        self, name, namespace, symbols, names=None, doc=None, other_props=None
    ):
...
class ArraySchema(Schema):
    ...
    Args:
      ...
      other_props:
    """
        super(ArraySchema, self).__init__(type=ARRAY, other_props=other_props)
...
class RecordSchema(NamedSchema):
    ...
    Return:
      The field schema.
    """
        field_schema = SchemaFromJSONData(json_data=field_desc["type"], names=names)
...
def _SchemaFromJSONObject(json_object, names):
    ...
    other_props = dict(FilterKeysOut(items=json_object, keys=SCHEMA_RESERVED_PROPS))
    ...
    if type in PRIMITIVE_TYPES:
        ...
        if type == ARRAY:
            ...
            return ArraySchema(
                items=SchemaFromJSONData(items_desc, names), other_props=other_props
            )
            ...
            if values_desc is None:
                raise SchemaParseException(
                    'Invalid map schema descriptor with no "values" : %r.' % json_object
                )
            return MapSchema(
            ...
            error_schemas = map(
                lambda desc: SchemaFromJSONData(desc, names=names), error_desc_list
            )
  ...
  """
    try:
        ...
        raise SchemaParseException(
            ...
            "Error message: %r." % (json_string, exn)
        )
