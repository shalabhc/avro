...
class RequestStreamingProducer(object):
    ...
    def __init__(self, message):
        ...
        self._length += (self._length / ipc.BUFFER_SIZE + 2) * ipc.BUFFER_HEADER_LENGTH
    ...
    def startProducing(self, consumer):
        ...
        while (
            self.length - self.total_bytes_sent > 0
            and not self.paused
    ...
    def write(self):
        ...
        self.write_buffer(
            self.message[
                self.total_bytes_sent : (self.total_bytes_sent + buffer_length)
            ]
        )
    ...
    def dataReceived(self, data):
        ...
        while len(self.recvd) >= ipc.BUFFER_HEADER_LENGTH:
            ...
            if len(self.recvd) < buffer_length + ipc.BUFFER_HEADER_LENGTH:
                break
            buffer = self.recvd[
                ipc.BUFFER_HEADER_LENGTH : buffer_length + ipc.BUFFER_HEADER_LENGTH
            ]
...
class TwistedHTTPTransceiver(object):
    ...
    def transceive(self, request):
        req_method = "POST"
        req_headers = {"Content-Type": ["avro/binary"], "Accept-Encoding": ["identity"]}
