...
class TestDataFile(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls._temp_dir = tempfile.TemporaryDirectory(prefix=cls.__name__, suffix=".tmp")
    ...
    """
        temp_file = tempfile.NamedTemporaryFile(
            dir=self._temp_dir.name, prefix="test", suffix=".avro", delete=False
        )
    ...
    def testAppend(self):
        ...
        for iexample, (writer_schema, datum) in enumerate(SCHEMAS_TO_VALIDATE):
            for codec in CODECS_TO_VALIDATE:
                ...
                logging.debug(
                    "Appended data has %d items: %r", len(appended_data), appended_data
                )
