...
_TEST_JSON_VALUE = {"first": "daffy", "last": "duck", "type": "duck"}
...
class TestCat(unittest.TestCase):
    ...
    def WriteAvroFile(file_path):
        ...
        with open(file_path, "wb") as writer:
            with avro.datafile.DataFileWriter(
                writer=writer, datum_writer=avro.io.DatumWriter(), writer_schema=schema
            ) as writer:
    ...
    def setUp(self):
        ...
        delete = True
        self._avro_file = tempfile.NamedTemporaryFile(
            prefix="test-", suffix=".avro", delete=delete
        )
    ...
    def testFields(self):
        ...
        self.assertEqual(
            json.loads(lines[0]), {"first": "daffy", "last": "duck", "type": "duck"}
        )
...
class TestWrite(unittest.TestCase):
    ...
    """
        return RunScript(
            "write", "--schema", self._schema_file.name, stdin=stdin, *args
        )
    ...
    def testStdin(self):
        with tempfile.NamedTemporaryFile(prefix="test-", suffix=".dat") as temp:
            with open(self._json_file.name, "rb") as input_content:
                with open(temp.name, "wb") as out:
                    out.write(
                        self._RunWrite("--input-type", "json", stdin=input_content)
                    )
