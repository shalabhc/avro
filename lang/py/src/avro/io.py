...
def validate(expected_schema, datum):
    ...
    if schema_type == "null":
        ...
        if (
            hasattr(expected_schema, "logical_type")
            and expected_schema.logical_type == "decimal"
        ):
    ...
    # Check for int, float, long and decimal
        ...
        return isinstance(datum, list) and False not in [
            validate(expected_schema.items, d) for d in datum
        ]
        ...
        return (
            ...
            and False
            not in [validate(expected_schema.values, v) for v in datum.values()]
        )
...
class DatumReader(object):
    ...
    def match_schemas(writers_schema, readers_schema):
        ...
        if "union" in [w_type, r_type] or "error_union" in [w_type, r_type]:
            ...
            writers_schema, readers_schema, ["fullname"]
        ):
            return True
            ...
            writers_schema, readers_schema, ["fullname", "size"]
        ):
            return True
            ...
            writers_schema.values, readers_schema.values, ["type"]
        ):
            return True
        elif w_type == r_type == "array" and DatumReader.check_props(
            writers_schema.items, readers_schema.items, ["type"]
        ):
            return True
    ...
    def read_data(self, writers_schema, readers_schema, decoder):
        ...
        if writers_schema.type == "null":
            ...
            if (
                hasattr(writers_schema, "logical_type")
