...
if inputProtocol is None:
    ...
    if not (os.path.exists(pfile)):
        raise Exception(
            "Could not locate the InputProtocol: {0} does not exist".format(pfile)
        )
...
if outputProtocol is None:
    ...
    if not (os.path.exists(pfile)):
        raise Exception(
            "Could not locate the OutputProtocol: {0} does not exist".format(pfile)
        )
  ...
  """
    ...
    """
        ...
        if partition is None:
            ...
            self.outputClient.request(
                "outputPartitioned", {"datum": self.buff.read(), "partition": partition}
            )
...
class TetherTask(object):
  ...
  """
    ...
    """
        ...
        self.log.info(
            "TetherTask.open: Opening connection to parent server on port={0}".format(
                clientPort
            )
        )
    ...
    """
        ...
        try:
            ...
            if taskType == TaskType.MAP:
                self.inReader = avio.DatumReader(
                    writers_schema=inSchema, readers_schema=self.inschema
                )
                ...
                self._red_fkeys = [
                    f.name for f in self.midschema.fields if not (f.order == "ignore")
                ]
        ...
        try:
            ...
            for i in range(count):
                if self.taskType == TaskType.MAP:
                    ...
                    if prev != None and not (
                        keys_are_equal(self.midRecord, prev, self._red_fkeys)
                    ):
    ...
    """
        ...
        raise NotImplementedError(
            "This is an abstract method which should be overloaded in the subclass"
        )
    ...
    """
        self.outputClient.request(
            "count", {"group": group, "name": name, "amount": amount}
        )
    ...
    """
        self.log.error(
            "TetherTask.fail: failure occured message follows:\n{0}".format(message)
        )
        try:
            ...
            estr = traceback.format_exc()
            self.log.error(
                "TetherTask.fail: an exception occured while trying to send the fail message to the output server:\n{0}".format(
                    estr
                )
            )
