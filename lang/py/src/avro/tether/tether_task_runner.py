    ...
    def invoke(self, message, request):
        try:
            if message.name == "configure":
                ...
                self.task.configure(
                    request["taskType"], request["inSchema"], request["outSchema"]
                )
                ...
                try:
                    ...
                    self.log.error(
                        "Exception occured while processing the partitions message: Message:\n"
                ...
                self.log.warning(
                    "TetherTaskRunner: recieved unknown message {0}".format(
                        message.name
                    )
                )
            ...
            self.log.error(
                "Error occured while processing message: {0}".format(message.name)
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        print(
            "Error: tether_task_runner.__main__: Usage: tether_task_runner task_package.task_module.TaskClass"
        )
