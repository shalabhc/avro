...
NAMED_TYPES = ("fixed", "enum", "record", "error")
...
class Schema(object):
    ...
    other_props = property(
        lambda self: get_other_props(self._props, SCHEMA_RESERVED_PROPS),
    ...
    def __init__(self, precision, scale=0):
        ...
        if not isinstance(precision, int) or precision <= 0:
            raise SchemaParseException(
                """Precision is required for logical type
            ...
            raise SchemaParseException(
                "Cannot store precision digits. Max is %s" % (max_precision)
            )
    ...
    def __init__(self, items, names=None, other_props=None):
        ...
        if isinstance(items, basestring) and names.has_name(items, None):
            ...
            try:
                ...
                fail_msg = (
                    "Items schema (%s) not a valid Avro schema: %s (known names: %s)"
                    % (items, e, names.names.keys())
                )
                raise SchemaParseException(fail_msg)
        ...
        for schema in schemas:
            if isinstance(schema, basestring) and names.has_name(schema, None):
                ...
                try:
                    ...
                    raise SchemaParseException(
                        "Union item must be a valid Avro schema: %s" % str(e)
                    )
    ...
    def make_field_objects(field_data, names):
        ...
        for i, field in enumerate(field_data):
            if callable(getattr(field, "get", None)):
                ...
                new_field = Field(
                    type, name, has_default, default, order, names, doc, other_props
                )
    ...
    if callable(getattr(json_data, "get", None)):
        ...
        if "logicalType" in json_data:
            ...
            if logical_type != "decimal":
                raise SchemaParseException(
                    "Currently does not support %s logical type" % logical_type
                )
        if type in PRIMITIVE_TYPES:
            if type == "bytes":
                if logical_type == "decimal":
                    ...
                    scale = (
                        0 if json_data.get("scale") is None else json_data.get("scale")
                    )
            ...
            if type == "fixed":
                ...
                if logical_type == "decimal":
                    ...
                    return FixedDecimalSchema(
                        size, name, precision, scale, namespace, names, other_props
                    )
                ...
                doc = json_data.get("doc")
                return RecordSchema(
                    name, namespace, fields, names, type, doc, other_props
                )
            else:
...
def parse(json_string):
    ...
    try:
        ...
        raise SchemaParseException(
            "Error parsing JSON: %s, error = %s" % (json_string, e)
        )
