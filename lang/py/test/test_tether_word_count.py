    ...
    """
        ...
        with file(fname, "w") as hf:
            ...
            writer = DataFileWriter(
                hf, avio.DatumWriter(inschema), writers_schema=schema.parse(inschema)
            )
...
"""
            ...
            osfile = tempfile.NamedTemporaryFile(
                mode="w", suffix=".avsc", prefix="wordcount", delete=False
            )
            ...
            args.append(
                os.path.abspath(
                    "@TOPDIR@/../java/tools/target/avro-tools-@AVRO_VERSION@.jar"
                )
            )
...
"""
            ...
            exhf = tempfile.NamedTemporaryFile(
                mode="w", prefix="exec_word_count_", delete=False
            )
            exfile = exhf.name
