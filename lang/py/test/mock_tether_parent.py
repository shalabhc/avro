  ...
  """
    ...
    def invoke(self, message, request):
        if message.name == "configure":
            print(
                "MockParentResponder: Recieved 'configure': inputPort={0}".format(
                    request["port"]
                )
            )
            ...
            print(
                "MockParentResponder: Recieved 'status': message={0}".format(
                    request["message"]
                )
            )
        elif message.name == "fail":
            print(
                "MockParentResponder: Recieved 'fail': message={0}".format(
                    request["message"]
                )
            )
        else:
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "start_server":
        ...
        print(
            "mock_tether_parent: Launching Server on Port: {0}".format(
                SERVER_ADDRESS[1]
            )
        )
